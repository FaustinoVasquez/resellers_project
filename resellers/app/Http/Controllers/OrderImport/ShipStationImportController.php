...
class ShipStationImportController extends Controller
{
    public function index()
    {
        return view('orderImport/shipStationImport');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return Redirect::to('ShipImport')->withInput()->with('error',$string);
        } else {
            if (Input::file('file')->isValid()) {
                ...
                try{
                  $query= DB::select("EXEC [Inventory].[dbo].[sp_ImportResellerShipStation] 'c:\\sharedb\\OrderImport\\".$fileName."','{$customerId}','{$po}'");                }
                catch(Exception $ex){
